...
const projectSettings = fs.readFileSync(path.join(projectPath, "ProjectSettings/ProjectSettings.asset"), "utf8");
const versionCodeMatch = projectSettings.match(/^\s*AndroidBundleVersionCode:\s*(\d+)\s*$/m);
if (!versionCodeMatch) throw new Error("Android bundle version code is missing.");
...
const output = path.join(workspacePath, `outputs/Crownfront-v${versionName}-code${versionCode}.aab`);
const logDir = path.join(workspacePath, `qa-logs/v${versionName}-code${versionCode}`);
const buildLog = path.join(workspacePath, `android-aab-build-${versionName.replace(".", "")}-code${versionCode}.log`);
...
const checks = {
  ...
  versionNameValid: manifest.includes(`android:versionName="${versionName}"`),
  versionCodeValid: manifest.includes(`android:versionCode="${versionCode}"`),
...
const report = { version: versionName, versionCode, generatedAt: new Date().toISOString(),
