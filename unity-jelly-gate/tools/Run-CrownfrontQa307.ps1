...
$qaDirectory = Join-Path $workspacePath 'qa-artifacts\Crownfront-QA-307'
$qaExecutable = Join-Path $qaDirectory 'Crownfront-QA.exe'
$logDirectory = Join-Path $workspacePath 'qa-logs\v1.00-code16'
...
if (-not $ReuseBuild -or -not (Test-Path -LiteralPath $qaExecutable)) {
    ...
    $buildSucceeded = $buildProcess.ExitCode -eq 0 -and
        (Test-Path -LiteralPath $buildLog) -and
    ...
    if (-not $buildSucceeded) {
        throw "CROWNFRONT code 16 QA build failed. See $buildLog"
    }
}
...
[ordered]@{ version='1.00'; versionCode=16; generatedAt=(Get-Date).ToString('o');
    passed=$passed; buildLog=$buildLog; probes=$results } |
    ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $summaryPath -Encoding utf8
if (-not $passed) { throw "CROWNFRONT code 16 QA failed. See $summaryPath" }
Write-Output "CROWNFRONT code 16 QA completed: $summaryPath"
