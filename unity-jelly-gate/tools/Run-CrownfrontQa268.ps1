...
$qaDirectory = Join-Path $workspacePath 'qa-artifacts\Crownfront-QA-268'
$qaExecutable = Join-Path $qaDirectory 'Crownfront-QA.exe'
$logDirectory = Join-Path $workspacePath 'qa-logs\v2.68'
...
if (-not $ReuseBuild -or -not (Test-Path -LiteralPath $qaExecutable)) {
    ...
    if ($build.ExitCode -ne 0 -or -not (Test-Path -LiteralPath $qaExecutable) -or $compileErrors.Count -gt 0) {
        throw "Crownfront v2.68 QA player build failed. See $buildLog"
    }
}

$probes = @(
    @{ Name = '01-release-blockers'; Argument = '-qaRelease268'; Pattern = 'QA_RELEASE_268 passed=True' },
    @{ Name = '02-shield-movement'; Argument = '-qaTankMovement267'; Pattern = 'QA_TANK_MOVEMENT_267 passed=True poses=24576 actors=48 metricMisses=0' },
    @{ Name = '03-every-enemy-sprite'; Argument = '-qaSprite264'; Pattern = 'QA_SPRITE_264 passed=True bossPoses=5760 regularPoses=10176' },
    @{ Name = '04-guide-portrait-frames'; Argument = '-qaGuideBossFrame266'; Pattern = 'QA_GUIDE_BOSS_FRAME_266 layout=True bodies=True' },
    @{ Name = '05-battlefield-regression'; Argument = '-qaBattlefield264'; Pattern = 'QA_BATTLEFIELD_264 passed=True wave=12-88 squad=4-10 burst=2-4 side=0.40' }
)
...
foreach ($probe in $probes) {
    ...
    $line = if ($null -ne $match) { $match.Line.Trim() } else {
        (Get-Content -LiteralPath $runtimeLog | Select-String -Pattern 'QA_' | Select-Object -Last 1).Line.Trim()
    }
    $results += [ordered]@{
...
$summary = [ordered]@{
    version = '2.68.1'; generatedAt = (Get-Date).ToString('o'); playerBuild = $qaExecutable
...
if (-not $summary.allQaPassed) { throw "One or more Crownfront v2.68 QA checks failed. See $summaryPath" }
Write-Output "Crownfront v2.68 QA completed. Summary: $summaryPath"
