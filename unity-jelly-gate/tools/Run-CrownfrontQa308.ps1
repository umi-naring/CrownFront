...
$qaDirectory = Join-Path $workspacePath 'qa-artifacts\Crownfront-QA-308'
$qaExecutable = Join-Path $qaDirectory 'Crownfront-QA.exe'
$logDirectory = Join-Path $workspacePath 'qa-logs\v1.00-code17'
...
if (-not $ReuseBuild -or -not (Test-Path -LiteralPath $qaExecutable)) {
    ...
    if (-not $buildSucceeded) { throw "CROWNFRONT code 17 QA build failed. See $buildLog" }
}

$probes = @(
    @{ Name='royal-ui-308'; Argument='-qaRoyalUi308'; Pattern='QA_ROYAL_UI_308 passed=True'; Graphics=$true },
...
[ordered]@{ version='1.00'; versionCode=17; generatedAt=(Get-Date).ToString('o');
    passed=$passed; buildLog=$buildLog; probes=$results } |
    ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $summaryPath -Encoding utf8
if (-not $passed) { throw "CROWNFRONT code 17 QA failed. See $summaryPath" }
Write-Output "CROWNFRONT code 17 QA completed: $summaryPath"
