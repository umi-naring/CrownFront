...
$qaDirectory = Join-Path $workspacePath 'tmp\Crownfront-QA-262'
$qaExecutable = Join-Path $qaDirectory 'Crownfront-QA.exe'
$logDirectory = Join-Path $workspacePath 'qa-logs\v2.62'
...
if (-not $ReuseBuild -or -not (Test-Path -LiteralPath $qaExecutable)) {
    ...
    if ($build.ExitCode -ne 0 -or -not (Test-Path -LiteralPath $qaExecutable) -or $compileErrors.Count -gt 0) {
        throw "Crownfront v2.62 QA player build failed. See $buildLog"
    }
}

$probes = @(
    @{ Name = '02-chairman-sprite-terrain'; Argument = '-qaRelease262'; Pattern = 'QA_RELEASE_262 passed=True' },
    @{ Name = '03-all-round-runtime'; Argument = '-qaRelease261'; Pattern = 'QA_RELEASE_261 passed=True' },
    @{ Name = '04-sprite-regression'; Argument = '-qaRelease260'; Pattern = 'QA_RELEASE_260 passed=True' },
    @{ Name = '05-all-unit-directions'; Argument = '-qaUltimateSpriteAudit256'; Pattern = 'QA_ULTIMATE_SPRITE_AUDIT_256 passed=True' },
    @{ Name = '06-navigation'; Argument = '-qaNavigationCombat'; Pattern = 'QA_NAVIGATION_COMBAT' },
    @{ Name = '07-guide'; Argument = '-qaGuide'; Pattern = 'QA_GUIDE modal=True tabs=True' }
)
...
$summary = [ordered]@{
    version = '2.62.0'; generatedAt = (Get-Date).ToString('o'); playerBuild = $qaExecutable
    allQaPassed = -not ($results.passed -contains $false); probes = $results
}
$summary | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $summaryPath -Encoding utf8
if (-not $summary.allQaPassed) { throw "One or more Crownfront v2.62 QA probes failed. See $summaryPath" }
Write-Output "Crownfront v2.62 QA completed. Summary: $summaryPath"
