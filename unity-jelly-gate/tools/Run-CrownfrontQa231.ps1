...
$qaDirectory = Join-Path $workspacePath 'tmp\Crownfront-QA-231'
$qaExecutable = Join-Path $qaDirectory 'Crownfront-QA.exe'
$buildLog = Join-Path $workspacePath 'qa-build-231-runner.log'
...
for ($pass = 1; $pass -le [Math]::Max(1, $Passes); $pass++) {
    $runtimeLog = Join-Path $workspacePath "qa-release-231-runner-pass$pass.log"
    $process = Start-Process -FilePath $qaExecutable `
        -ArgumentList @('-batchmode', '-nographics', '-qaRelease231', '-logFile', $runtimeLog) `
        -WindowStyle Hidden -Wait -PassThru
    $result = Get-Content -LiteralPath $runtimeLog | Select-String -Pattern 'QA_RELEASE_231' |
...
Write-Output "Crownfront v2.31.0 QA completed successfully."
