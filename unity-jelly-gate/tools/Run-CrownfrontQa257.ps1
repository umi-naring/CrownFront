...
$qaDirectory = Join-Path $workspacePath 'tmp\Crownfront-QA-257'
$qaExecutable = Join-Path $qaDirectory 'Crownfront-QA.exe'
$logDirectory = Join-Path $workspacePath 'qa-logs\v2.57'
...
if (-not $ReuseBuild -or -not (Test-Path -LiteralPath $qaExecutable)) {
    ...
    if ($build.ExitCode -ne 0 -or -not (Test-Path -LiteralPath $qaExecutable) -or $compileErrors.Count -gt 0) {
        throw "Crownfront v2.57 QA player build failed. See $buildLog"
    }
}

$probes = @(
    @{ Name = '01-default-skin-commerce'; Argument = '-qaDefaultSkinCommerce257'; Pattern = 'QA_DEFAULT_SKIN_COMMERCE_257 passed=True' },
    ...
    @{ Name = '04-boss-archer-google'; Argument = '-qaBossArcherGoogle254'; Pattern = 'QA_BOSS_ARCHER_GOOGLE_254 passed=True' },
    @{ Name = '05-presentation'; Argument = '-qaPresentation253'; Pattern = 'QA_PRESENTATION_253 passed=True' },
    @{ Name = '06-boss-directional'; Argument = '-qaBossDirectional248'; Pattern = 'QA_BOSS_DIRECTIONAL_248 loaded=True' },
    @{ Name = '07-vfx'; Argument = '-qaVfx245'; Pattern = 'QA_VFX_245' },
    @{ Name = '08-augment-bronze'; Argument = '-qaAugmentBronze252'; Pattern = 'QA_AUGMENT_TIER_252 tier=Bronze passed=True' },
    @{ Name = '09-augment-silver'; Argument = '-qaAugmentSilver252'; Pattern = 'QA_AUGMENT_TIER_252 tier=Silver passed=True' },
    @{ Name = '10-augment-gold'; Argument = '-qaAugmentGold252'; Pattern = 'QA_AUGMENT_TIER_252 tier=Gold passed=True' },
    @{ Name = '11-augment-platinum'; Argument = '-qaAugmentPlatinum252'; Pattern = 'QA_AUGMENT_TIER_252 tier=Platinum passed=True' },
    @{ Name = '12-augment-diamond'; Argument = '-qaAugmentDiamond252'; Pattern = 'QA_AUGMENT_TIER_252 tier=Diamond passed=True' },
    @{ Name = '13-sprite-range'; Argument = '-qaSpriteRange252'; Pattern = 'QA_SPRITE_RANGE_252 passed=True' },
    @{ Name = '14-special-enemies'; Argument = '-qaSpecialEnemies252'; Pattern = 'QA_SPECIAL_ENEMIES_252 passed=True' },
    @{ Name = '15-monetization'; Argument = '-qaMonetization252'; Pattern = 'QA_MONETIZATION_252 structural=True' },
    @{ Name = '16-combat'; Argument = '-qaCombatSystems'; Pattern = 'QA_COMBAT_SYSTEMS' },
    @{ Name = '17-performance'; Argument = '-qaPerformance240'; Pattern = 'QA_PERFORMANCE_240' }
)
...
$summary = [ordered]@{
    version = '2.57.0'; generatedAt = (Get-Date).ToString('o'); playerBuild = $qaExecutable
    allQaPassed = -not ($results.passed -contains $false); probes = $results
}
$summary | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $summaryPath -Encoding utf8
if (-not $summary.allQaPassed) { throw "One or more Crownfront v2.57 QA probes failed. See $summaryPath" }
Write-Output "Crownfront v2.57 QA completed. Summary: $summaryPath"
