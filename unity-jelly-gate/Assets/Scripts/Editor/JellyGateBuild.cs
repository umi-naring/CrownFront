...
namespace JellyGate.Editor
{
    ...
    public static class JellyGateBuild
    {
        ...
        public static void ConfigureProject()
        {
            ...
            // Keep the user-facing version name at 1.00 and publish this economy/refund update as code 28.
        ...
        public static void BuildAndroidAppBundle()
        {
            ...
            if (PlayerSettings.bundleVersion != "1.00" || PlayerSettings.Android.bundleVersionCode != 28)
                throw new BuildFailedException("CROWNFRONT release settings must be version 1.00 (code 28) before building.");
            ...
            var outputPath = ReadArgument("-outputPath");
            if (string.IsNullOrWhiteSpace(outputPath))
                outputPath = Path.GetFullPath(Path.Combine(Application.dataPath, "../../outputs/Crownfront-v1.00-code28.aab"));
            var outputDirectory = Path.GetDirectoryName(outputPath);
            if (!string.IsNullOrEmpty(outputDirectory)) Directory.CreateDirectory(outputDirectory);
